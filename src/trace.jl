...
function record!(sampler::Stan{T}, trace::NamedTuple, info, iteration, chain) where {T}
    keys = (:accepted, :acceptstat, :divergence, :energy, :leapfrog, :stepsize, :treedepth)
    for k in keys
        if haskey(info, k)
...
function record!(
    ...
    keys = (
        :accepted, :acceptstat, :damping, :divergence, :drift, :energy, :noise, :stepsize
    )
...
function record!(sampler::MALA{T}, trace::NamedTuple, info, iteration, chain) where {T}
    keys = (:accepted, :acceptstat, :divergence, :energy, :stepsize)
    for k in keys
        if haskey(info, k)
            trace[k][iteration, chain] = info[k]
        end
...
function record!(
    ...
    keys = (:accepted, :acceptstat, :divergence, :energy)
    for k in keys
        if haskey(info, k)
            trace[k][iteration, chain] = info[k]
        end
